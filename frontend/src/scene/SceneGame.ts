...
class SceneGame extends egret.DisplayObjectContainer {
  ...
  init() {
    ...
    this.barWrap.addChild(this.heightBar);
    this.barWrap.x = 35;
    ...
    if (GameData.number == 1) {
      ...
      oppCar.x = 482 + UIConfig.offsetW;
    } else {
      myCar.x = 482 + UIConfig.offsetW;
      oppCar.x = 276 + UIConfig.offsetW;
    }
    myCar.y = - 300;
    oppCar.y = - 300;
  ...
  createTree(start = 0) {
    ...
    for (let i = 0; i < treeMax; i++) {
      let tree = new Bitmap(`pic_lb_${Util.RandomNumBoth(1, 11)}_png`)
  ...
  createNpcCar() {
    ...
    for (let i = 0; i < carLen1; i++) {
      let npc = new Car(Util.RandomNumBoth(4, 6))
  ...
  private setProgress() {
    ...
    this.prgBlue.y = myCar.y * 632 / totalLen + 632
    this.prgRed.y = oppCar.y * 632 / totalLen + 632
